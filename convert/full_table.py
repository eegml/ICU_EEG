...
# %% [markdown]
# There are some subjects in the repository which were not ultimately used in the published form of this project (in particular, anything with 'IIC' in it - these were interictal - ictal continuum patients which we did not want to use in training the model), as well as a few others which clinicians on our team judged to have insufficient data quality. We also had to re-upload files recently under new IDs on ieeg.org for deidentification purposes which may have made some of the annotation files fail. 
#
